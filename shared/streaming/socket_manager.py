    ...
    async def connect(self) -> None:
        ...
        try:
            ...
            self._websocket = await connect_fn(
                ...
                compression=None,        # disable per-message compression (latency + CPU)
                close_timeout=8,         # allow a bit more time for a clean TCP close
                open_timeout=12,         # increase handshake timeout for initial connect
                max_queue=None,          # unlimited incoming queue (avoid backpressure drops)
            )
