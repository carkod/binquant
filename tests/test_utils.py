...
def test_supress_trailling():
    assert math.isclose(supress_trailling("3.1400000"), 3.14, rel_tol=1e-9)
    assert math.isclose(supress_trailling(2.05e-5), 2.1e-5, rel_tol=1e-9)
    assert math.isclose(supress_trailling(3.140), 3.14, rel_tol=1e-9)
...
def test_supress_notation():
    ...
    assert supress_notation(123.456, 2) == "123.46"
    # Compare only the first 8 characters for precision-sensitive case
    assert supress_notation(0.0001234, -1)[:8] == "0.000123"
