...
class TestAutotradeConsumer:
    ...
    @pytest.mark.asyncio
    async def test_terminal_entry_rejection_is_not_logged_twice_or_deactivated(self):
        settings = make_autotrade_settings(autoswitch=False)
        signal = SignalsConsumer(autotrade=True, current_price=100)
        self.mock_binbot_api.activate_bot.return_value = BotResponse(
            error=1,
            message="Entry rejected: expected fill is outside the reference band.",
            data=BotModel(
                id=BOT_ID,
                pair="BTCUSDT",
                status=Status.error,
        ...
        with pytest.raises(AutotradeError, match="outside the reference band"):
            await autotrade.activate_autotrade(signal)

        self.mock_binbot_api.submit_bot_event_logs.assert_not_called()
