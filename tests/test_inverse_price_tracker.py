...
async def test_inverse_price_tracker_skips_when_insufficient_rows(monkeypatch):
    ...
    algo.telegram_consumer.send_signal.assert_not_awaited()
    algo.at_consumer.process_autotrade_restrictions.assert_not_awaited()
...
async def test_inverse_price_tracker_emits_in_trend_up_market(monkeypatch):
    ...
    await algo.signal(close_price=100.0, bb_high=101.0, bb_low=99.0, bb_mid=100.0)
    algo.telegram_consumer.send_signal.assert_awaited_once()
    algo.at_consumer.process_autotrade_restrictions.assert_awaited_once()
...
async def test_inverse_price_tracker_skips_in_trend_down_or_high_stress(monkeypatch):
    ...
    algo_trend_down.telegram_consumer.send_signal.assert_not_awaited()
    ...
    algo_high_stress.telegram_consumer.send_signal.assert_not_awaited()
...
async def test_inverse_price_tracker_telegram_message_contains_expected_text(
    ...
    await algo.signal(100.0, 101.0, 99.0, 100.0)
    msg = algo.telegram_consumer.send_signal.await_args.args[0]
